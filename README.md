...
- **Value props** (`valueProps`): concise cards explaining what makes myrug different.
...
- Current destination: `https://myrug.co.za/gallery`
...
- `metadataBase` is set to `https://myrug.co.za` in `app/layout.tsx`.
...
- `app/manifest.json`:
  - Uses `name` / `short_name = "myrug"`
  - `theme_color` matches the default `theme.brand` (`#8B5E34`).
